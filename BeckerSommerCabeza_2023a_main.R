...
#### working Title:Neural Mechanisms of Creative Problem Solving: From Representational Change to Memory Formation
#### (c) almaxi@gmail.com
...
newdata = PPSS[ 
  ...
  ERS_M0a_IME<- lmer(ERS_PrePost_R ~ cor2_fac+RT+                                       +ROI_bilat+sessionblock+(1|ID)+ (1|Item), data= ERSdata, na.action  = na.omit)
  ERS_M0b_IME<- lmer(ERS_PrePost_R ~ cor2_fac+RT+               insight_mediansplit_fac +ROI_bilat+sessionblock+(1|ID)+ (1|Item), data= ERSdata, na.action  = na.omit)
  ERS_M1_IME <- lmer(ERS_PrePost_R ~ cor2_fac+RT+SME_solve_fac+ insight_mediansplit_fac +ROI_bilat+sessionblock+(1|ID)+ (1|Item), data= ERSdata, na.action  = na.omit)
  ERS_M2_IME <- lmer(ERS_PrePost_R ~ cor2_fac+RT+SME_solve_fac* insight_mediansplit_fac +ROI_bilat+sessionblock+(1|ID)+ (1|Item), data= ERSdata, na.action  = na.omit)
  ERS_M3_IME <- lmer(ERS_PrePost_R ~ cor2_fac+RT+SME_solve_fac* insight_mediansplit_fac *ROI_bilat+sessionblock+(1|ID)+ (1|Item), data= ERSdata, na.action  = na.omit)
  
  hist(residuals(ERS_M2_IME)) # residuals are normally distributed -> model is adecuate
  anova(ERS_M0a_IME,ERS_M0b_IME, ERS_M1_IME, ERS_M2_IME,ERS_M3_IME)#, RSA4)#,  M4)#
  ggpredict(ERS_M0b_IME , c(  'ROI_bilat','insight_mediansplit_fac')) %>% plot(show.title = F) + ggplot2::theme_classic()
...
#### 3b1a) RSA2-IME using AlexNet8: Insight memory effect for representations #####
  ...
  M0AN_mem <- lmer(RSA_AlexNet8_realpix_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_sum+                ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
  M1AN_mem <- lmer(RSA_AlexNet8_realpix_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_sum+SME_solve_fac+  ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
  M2aAN_mem <-lmer(RSA_AlexNet8_realpix_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_sum+tp_fac*SME_solve_fac+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
  M2bAN_mem <-lmer(RSA_AlexNet8_realpix_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_sum+tp_fac*SME_solve_fac+SME_solve_fac*insight_sum+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
  M2cAN_mem <-lmer(RSA_AlexNet8_realpix_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_sum*SME_solve_fac+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
  M3AN_mem <- lmer(RSA_AlexNet8_realpix_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_sum*SME_solve_fac*ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
  
  #hist(residuals(M1AN_mem)) # residuals are normally distributed -> model is adecuate
  anova(M0AN_mem, M1AN_mem,M2aAN_mem,M2bAN_mem, M2cAN_mem,M3AN_mem)
  tab_model(M2cAN_mem, show.std =T)
...
#### 3b2a) RSA2-Insight*Memory using W2V: Insight memory effect for representations  #####
 ...
 RSAdata = newdata[ !is.na(newdata$SME_solve_fac) & newdata$RSA_W2V_control_corr_n_vis > 10 & newdata$tp != 2,]
    ...
    M0W2V_mem <- lmer(RSA_W2V_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_mediansplit+                ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M1W2V_mem <- lmer(RSA_W2V_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_mediansplit+SME_solve_fac+  ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M2aW2V_mem <-lmer(RSA_W2V_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_mediansplit+tp_fac*SME_solve_fac+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M2bW2V_mem <-lmer(RSA_W2V_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_mediansplit+tp_fac*SME_solve_fac+SME_solve_fac*insight_mediansplit+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M2cW2V_mem <-lmer(RSA_W2V_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_mediansplit*SME_solve_fac+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M3W2V_mem <- lmer(RSA_W2V_R_vis ~sessionblock+cor2_fac+RT+tp_fac*insight_mediansplit*SME_solve_fac*ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    
    hist(residuals(M2aW2V_mem)) # residuals are normally distributed -> model is adecuate
    anova(M0W2V_mem, M1W2V_mem, M2aW2V_mem,M2bW2V_mem,M2cW2V_mem,M3W2V_mem)#,
    tab_model(M2cW2V_mem, show.std =T)
    summary(M2W2V_mem)
...
#### 5) Plotting all results together #####################     
    ...
    IME_AN8_W2V_Amy_hC <- ggarrange(ERS_Mem_finalPlot, AN08_Mem_finalPlot,IME_Amy_HC_ggplot_finalPlot, W2V_Mem_finalPlot,
