...
#### load main data #####################################################
#setwd('G:/Meine Ablage/studies/Maxi/IN_REVISION/PVI/GitHub_4_publication/REVISION1/data') 
setwd('C:/Users/CabezaLab-Berlin1/Google Drive/studies/Maxi/IN_REVISION/PVI/GitHub_4_publication/REVISION1/data') 
...
#### Measurement Model: AHA Experience #####
  ...
  #### Behavioral Results - descriptives ####
    ...
    tab_model(M1_cor2, show.std = T)
        ...
        ### test if aha differs between correctly and incorrectly solved trials
        data_new = PPSS[ PPSS$ROI == 'l_Amygdala' & PPSS$tp == 1 & (PPSS$RT>=1.5 & PPSS$RT<=9.5),]
        M0_aha <- lmer(insight_sum ~         (1|ID) + (1|Item),data= data_new, na.action  = na.omit)
        M1_aha <- lmer(insight_sum ~ cor2   + (1|ID) + (1|Item),data= data_new, na.action  = na.omit)
        anova(M0_aha, M1_aha)
        tab_model(M1_aha, show.std = T)
  ...
  #####bootstrapping p-values for memory analysis
    ...
    boot_function_nS <- function(data1, indices) {
      d <- data1[indices, ]
      fit <- glmer(SME_solve_fac ~ sessionblock + insight_ms + (1 | ID) + (1 | Item), data = d,  family = binomial(link = "logit"), na.action  = na.omit)
        return(fixef(fit)["insight_ms"]) # insight_msLO-I_vs_nS Extract the custom contrast
       }
    
    # Perform bootstrapping with 1000 replications
    set.seed(123)  # Set seed for reproducibility
    results_insight_fac2 <- boot(data = data_new, statistic = boot_function_nS,  R = 1000, stype = "i",  sim = "ordinary", parallel= c("multicore"), ncpus=7)
    ...
    p_value <- mean(results_boot_insightsum$t <= 0)  # One-tailed test
    p_value_two_tailed <- 2 * min(mean(results_boot_insightsum$t <= 0), mean(results_boot_insightsum$t > 0))  # Two-tailed test
...
######## Exploratory: which dimension is driving the univariate insight effect? (standard space, single trial analysis) ######
    ...
    Activity_M10a <- lmer(Beta_Buttonpress ~ xRT +sessionblock +            +SME_solve_fac+(1|ID) + (1|Item),data= Activity_data[Activity_data$ROI_bilat== "aHC",], na.action  = na.omit)#Amy,aHC, pHC
    Activity_M11a <- lmer(Beta_Buttonpress ~ xRT +sessionblock +insight_sum*SME_solve_fac+(1|ID) + (1|Item),data= Activity_data[Activity_data$ROI_bilat== "aHC",], na.action  = na.omit)#Amy,aHC, pHC
    ...
    Activity_M10b <- lmer(Beta_Buttonpress ~ xRT +sessionblock +            +SME_solve_fac+(1|ID) + (1|Item),data= Activity_data[Activity_data$ROI_bilat== "pHC",], na.action  = na.omit)#Amy,aHC, pHC
    Activity_M11b <- lmer(Beta_Buttonpress ~ xRT +sessionblock +insight_sum*SME_solve_fac+(1|ID) + (1|Item),data= Activity_data[Activity_data$ROI_bilat== "pHC",], na.action  = na.omit)#Amy,aHC, pHC
    anova( Activity_M10b, Activity_M11b)
    tab_model(Activity_M11b, show.std =T)
      # permutation test
      Mperm_pHC1 =permlmer(Activity_M10b,Activity_M11b, plot = T, seed = 123)
      Mperm_pHC2 =permmodels(Activity_M11b, type = 2, seed = 123)
      
    Activity_M10c <- lmer(Beta_Buttonpress ~ xRT +sessionblock +            +SME_solve_fac+(1|ID) + (1|Item),data= Activity_data[Activity_data$ROI_bilat== "Amy",], na.action  = na.omit)#Amy,aHC, pHC
    Activity_M11c <- lmer(Beta_Buttonpress ~ xRT +sessionblock +insight_sum*SME_solve_fac+(1|ID) + (1|Item),data= Activity_data[Activity_data$ROI_bilat== "Amy",], na.action  = na.omit)#Amy,aHC, pHC
    anova( Activity_M10c, Activity_M11c)
      # permutation test
      Mperm_Amy1 =permlmer(Activity_M10c,Activity_M11c, plot = T, seed = 123)
      Mperm_Amy2 =permmodels(Activity_M11c, type = 2, seed = 123)

    # Define a function for bootstrapping
    #library(boot)
    boot_func_BB_insightsum <- function(data1, indices) {
      d <- data1[indices, ]
      fit <- lmer(Beta_Buttonpress ~ xRT +sessionblock +insight_sum*SME_solve_fac+(1|ID) + (1|Item),data= d, na.action  = na.omit)
      return(fixef(fit)["insight_sum:SME_solve_fac"]) # Extract the custom contrast
    }
    # Perform bootstrapping with 1000 replications
    set.seed(123)  # Set seed for reproducibility
    results_insight_sum <- boot(data = Activity_data[Activity_data$ROI_bilat== "aHC",], statistic = boot_func_BB_insightsum,  R = 1000, stype = "i",  sim = "ordinary",
                                parallel=c("multicore"), ncpus = 5)
    print(results_insight_sum); hist(results_insight_sum$t)
    exp(mean(results_insight_sum$t)) # mean odds ratio [exp(log odds)]  for HI-I > Lo-I
    boot.ci(results_insight_sum, conf= c(.95), type= c("perc")) # CIs of mean odds ratio -> here in log-odds
...
newdata = PPSS[ 
  ...
  tab_model(ERS_M2_IME_posthoc, show.std = T)
    ...
    permERS_M2_IME_posthoc1 = permmodels(ERS_M2_IME_posthoc, type = 2, seed = 123)
  
########  3b) RSA2-Insight  ####################################################
  newdata = PPSS[ 
    (PPSS$ROI == "r_aHC") & #pFusG" |PPSS$ROI_bilat == "iLOC" )&
...
#### 3b1) RSA2-Insight using AlexNet8 ######
  ...
  M0AN <- lmer(RSA_AlexNet8_realpix_R_vis ~ xRT+tp_fac                     +sessionblock +(1|ID)+ (1|Item), na.action  = na.omit,data= RSAdata)
  M1AN <- lmer(RSA_AlexNet8_realpix_R_vis ~ xRT+tp_fac+insight_sum+ROI_bilat+sessionblock +(1|ID)+ (1|Item), na.action  = na.omit,data= RSAdata) #cor2_fac +RT
  ...
  M3AN <- lmer(RSA_AlexNet8_realpix_R_vis ~ xRT+tp_fac*insight_sum*ROI_bilat+sessionblock +(1|ID)+ (1|Item), na.action  = na.omit,data= RSAdata)
  ...
  tab_model(M0AN, show.std = T) # to test effect fo solution time on repr.strength
...
#### 3b1a) RSA2-IME using AlexNet8: Insight memory effect for representations #####
  ...
  permAN_M3_IME = permlmer(M2aAN_mem,M2bAN_mem, plot = T, seed = 123)
  permAN_M4_IME = permlmer(M2bAN_mem,M2cAN_mem, plot = T, seed = 123)  
  permAN_M5_IME = permlmer(M2aaAN_mem,M2dAN_mem, plot = T, seed = 123)
...
#### 3b2a) RSA2-Insight*Memory using W2V: Insight memory effect for representations  #####
 ...
 RSAdata$xRT = scale(RSAdata$RT)
    ...
    M1W2V_mem <- lmer(RSA_W2V_R_vis ~Beta_Buttonpress+sessionblock+tp_fac+insight_sum              +ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M2W2V_mem <- lmer(RSA_W2V_R_vis ~Beta_Buttonpress+sessionblock+tp_fac+insight_sum+SME_solve_fac+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M3W2V_mem <- lmer(RSA_W2V_R_vis ~Beta_Buttonpress+sessionblock+tp_fac*SME_solve_fac+insight_sum+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M3Waa2V_mem<-lmer(RSA_W2V_R_vis ~Beta_Buttonpress+sessionblock+tp_fac*insight_sum+SME_solve_fac+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M3aW2V_mem<- lmer(RSA_W2V_R_vis ~Beta_Buttonpress+sessionblock+tp_fac+insight_sum+SME_solve_fac+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M4W2V_mem <- lmer(RSA_W2V_R_vis ~Beta_Buttonpress+sessionblock+tp_fac*insight_sum+tp_fac*SME_solve_fac+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M5W2V_mem <- lmer(RSA_W2V_R_vis ~Beta_Buttonpress+sessionblock+tp_fac*insight_sum*SME_solve_fac+ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    M6W2V_mem <- lmer(RSA_W2V_R_vis ~Beta_Buttonpress+sessionblock+tp_fac*insight_sum*SME_solve_fac*ROI_bilat+(1|ID)+ (1|Item),data= RSAdata, na.action  = na.omit)
    ...
    permW2V_M4_IME = permlmer(M3aW2V_mem,M5W2V_mem, plot = T, seed = 123)  
    permW2V_M5_IME = permlmer(M4W2V_mem,M5W2V_mem, plot = T, seed = 123)  
    permW2V_M5_IMEa = permmodels(M5W2V_mem, type = 2, seed = 123)
