...
#### SUPPLEMENT: Measurement Model: AHA Experience #####
  ...
  #####bootstrapping data for memory analysis
    ...
    #  Fig.2-B for display purproses (without RT)
     IME_ggpredict =ggpredict(M1_IME , c(  'insight_ms'))
...
##### 2a) Amy & HC: Insight: with single trials betas (standard space) #####  
    ...
    #PostHoc Test
    library("modelbased")
    estimate_slopes(Activity_M3, trend = "insight_sum", at = "ROI_bilat_ord", seed = 123)#,
...
######## Exploratory: which dimension is driving the univariate insight effect? (standard space, single trial analysis) ######
    newdata = PPSS[  PPSS$ROI_bilat == "Amy",] #aHC, pHC, Amy
    ...
    Activity_M10a <- lmer(Beta_Buttonpress ~ xRT +sessionblock +insight_sum+SME_solve_fac+(1|ID) + (1|Item),
                          data= Activity_data[Activity_data$ROI_bilat== "aHC",], na.action  = na.omit)#Amy,aHC, pHC
...
###### SUPPLEMENT: control analysis - Check for univariate activity effect in VOTC-RC Areas ########
    ...
    library("modelbased")
    estimate_slopes(Activity_VS_V6, trend = "insight_sum", at = "ROI_bilat",seed = 123)
...
######## 3a) RSA1-Insight - Multivoxel Pattern Similarity ######################################################
  ...
  tab_model(ERS_M2s, show.std  = T)
    ...
    #post-hoc
    ERSdata_BP = ERSdata[newdata$ROI_bilat == "pFusG" ,]# iLOC
...
newdata = PPSS[ 
  ...
  ERS_M0_IME_posthoc <- lmer(ERS_PrePost_R ~ xRT+insight_sum+              sessionblock+(1|ID)+ (1|Item),data= ERSdata[ERSdata$ROI_bilat== "pFusG",], na.action  = na.omit) 
  ERS_M1_IME_posthoc <- lmer(ERS_PrePost_R ~ xRT+insight_sum+SME_solve_fac+sessionblock+(1|ID)+ (1|Item),data= ERSdata[ERSdata$ROI_bilat== "pFusG",], na.action  = na.omit) 
  ERS_M2_IME_posthoc <- lmer(ERS_PrePost_R ~ xRT+insight_sum*SME_solve_fac+sessionblock+(1|ID)+ (1|Item),data= ERSdata[ERSdata$ROI_bilat== "pFusG",], na.action  = na.omit) 
...
#### 3b1) RSA2-Insight using AlexNet8 ######
  ...
  M1AN_posthoc <- lmer(RSA_AlexNet8_realpix_R_vis ~ tp_fac+insight_sum+sessionblock +(1|ID)+ (1|Item), na.action  = na.omit,data= RSAdata[RSAdata$ROI_bilat=="pFusG",])
  M2AN_posthoc <- lmer(RSA_AlexNet8_realpix_R_vis ~ tp_fac*insight_sum+sessionblock +(1|ID)+ (1|Item), na.action  = na.omit,data= RSAdata[RSAdata$ROI_bilat=="pFusG",])
...
#### 3b1a) RSA2-IME using AlexNet8: Insight memory effect for representations #####
  ...
  #interchange Beta_Buttonpress for xRT for analysis in Supplement p.9
...
#### 3b2) RSA2-Insight using W2V (word2vec) ################
  ...
  M1W2V_posthoc <- lmer(RSA_W2V_R_vis ~ xRT+tp_fac+insight_sum+sessionblock  +(1|ID)+ (1|Item),data= RSAdata[RSAdata$ROI_bilat=="pFusG",], na.action  = na.omit)
  M2W2V_posthoc <- lmer(RSA_W2V_R_vis ~ xRT+tp_fac*insight_sum+sessionblock  +(1|ID)+ (1|Item),data= RSAdata[RSAdata$ROI_bilat=="pFusG",], na.action  = na.omit)
  anova(M1W2V_posthoc,M2W2V_posthoc)
  tab_model(M2W2V_posthoc, show.std = T)
  
  # Supplement p.10 -> when Beta_Buttonpress instead of xRT is modelled   #library("modelbased")
  estimate_slopes(M2W2V, trend = "insight_sum", at = "tp_fac", seed = 123)
...
#### 3b2a) RSA2-Insight*Memory using W2V: Insight memory effect for representations  #####
 ...
 RSAdata$xRT = scale(RSAdata$RT)
    ...
    #Posthoc (note, substitute iLOC or pFusG)
    #M4W2V_mem_posthoc <- lmer(RSA_W2V_R_vis ~ xRT+tp_fac+insight_sum+SME_solve_fac+sessionblock  +(1|ID)+ (1|Item),data= RSAdata[RSAdata$ROI_bilat=="pFusG",], na.action  = na.omit)
    #M5W2V_mem_posthoc <- lmer(RSA_W2V_R_vis ~ xRT+tp_fac*insight_sum*SME_solve_fac+sessionblock  +(1|ID)+ (1|Item),data= RSAdata[RSAdata$ROI_bilat=="pFusG",], na.action  = na.omit)
    #anova(M4W2V_mem_posthoc,M5W2V_mem_posthoc)
    #tab_model(M5W2V_mem_posthoc, show.std = T)
